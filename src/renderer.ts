...
export function renderSong(song: Song): HTMLElement {
  ...
  const ro = new ResizeObserver(() => {
    ...
    secNodes.forEach((secNode, idx) => {
      const rowsPlain = secNode.querySelector(".cancionero-rows-plain") as HTMLElement;
  ...
  function renderPlainSection(container: HTMLElement, rows: SongRow[]) {
    ...
    const maxCols = Math.max(1, Math.floor(container.getBoundingClientRect().width / chW) - 2);

    for (const row of rows) {
      const chordLine = row.chordLine.replace(/\|/g, " ");
      const toks = [...row.chords].sort((a, b) => a.startCol - b.startCol);
      ...
      for (const seg of segments) {
        ...
        chordsPre.appendChild(buildChordFragment(chordLine, toks, seg.start, seg.end));
  ...
  function buildChordFragment(line: string, toks: ChordToken[], start: number, end: number): DocumentFragment {
    ...
    for (const t of toks) {
      ...
      // text before token
      if (tStart > cursor) frag.appendChild(document.createTextNode(line.slice(cursor, Math.min(tStart, end))));

      // token clipped to window
      const s = Math.max(tStart, start);
      const e = Math.min(tEnd, end);
      const strong = document.createElement("strong");
      strong.textContent = line.slice(s, e).replace(/b/g, "♭").replace(/#/g, "♯");
      frag.appendChild(strong);

      cursor = e;
    }
    // trailing text
    if (cursor < end) frag.appendChild(document.createTextNode(line.slice(cursor, end)));
    return frag;
  }

  function wrapPair(chords: string, lyrics: string, maxCols: number): Array<{ start: number; end: number }> {
  ...
  function renderChartLine(barTexts: string[], badgeIndex?: number, badgeRepeat?: number): HTMLElement {
  ...
  function setMode(m: Mode) { root.setAttribute("data-mode", m); }
...
/* utils */
function textSpan(t: string): HTMLSpanElement { const s = document.createElement("span"); s.textContent = t; return s; }
